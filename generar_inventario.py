...
<style>
  ...
  *{ box-sizing:border-box; margin:0; padding:0; }
  ...
  .dot.d-av{ background:#1e9e5a; } .dot.d-res{ background:#f59e0b; }
...
<div class="wrap">
  ...
  <div class="card" style="margin-bottom:16px;">
    <h3>Stock Real por Modelo <span class="sr-legend"><span class="dot d-av"></span>Disponible <span class="dot d-res"></span>Reservado</span></h3>
...
unidades.forEach(u => {
  const sm = shortModel(u.modelo);
  if (!stockModelo[sm]) stockModelo[sm] = { total: 0, res: 0 };
  stockModelo[sm].total++;
  if (isReserv(u)) stockModelo[sm].res++;
});
function stockRowHtml(nombre, d) {
  const av = d.total - d.res;
  const pAv = d.total ? (av / d.total * 100).toFixed(1) : 0;
  const pRes = d.total ? (d.res / d.total * 100).toFixed(1) : 0;
  return `<div class="stock-row">
    ...
    <span class="stock-nums"><b>${av}</b> disponibles · <b class="r">${d.res}</b> reservados</span>
  </div>`;
}
const srTotal = { total: 0, res: 0 };
Object.entries(stockModelo).forEach(([m, d]) => { srTotal.total += d.total; srTotal.res += d.res; });
document.getElementById("stockReal").innerHTML =
  ...
  `<div class="stock-row" style="border-top:2px solid var(--border);padding-top:10px;font-weight:700">
    <span class="stock-name">TOTAL (${srTotal.total} unidades)</span>
    <div class="stock-bar">
      <div class="stock-av" style="width:${((srTotal.total - srTotal.res) / srTotal.total * 100).toFixed(1)}%"></div>
      <div class="stock-res" style="width:${(srTotal.res / srTotal.total * 100).toFixed(1)}%"></div>
    </div>
    <span class="stock-nums" style="font-weight:800"><b>${srTotal.total - srTotal.res}</b> disponibles · <b class="r">${srTotal.res}</b> reservados</span>
  </div>`;
